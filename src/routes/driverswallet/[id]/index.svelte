<script context="module">
	export async function load({ fetch, params }) {
		const res = await fetch(`https://taxinetghana.xyz/drivers_wallet_detail/${params.id}/`, {
			headers: {
...
<script>
	...
	const handleUpdateRequest = async () => {
		...
		const apiUrl = `https://taxinetghana.xyz/update_drivers_wallet/${detailWallet.id}/`;
		axios({
			method: 'PUT',
			url: apiUrl,
			data: {
				amount: parseFloat(oldAmount) + parseFloat(amount),
				driver: detailWallet.driver
			},
...
<svelte:head>
	<title>Taxinet | {detailWallet.get_drivers_name}</title>
</svelte:head>
<DashboardHeader />
...
<div class="mainbox">
	<section class="inventorydetailcontainer">
		...
		>
			<a href="/driverswallets" class="close">X</a>
			<div class="picname">
				<img src={detailWallet.get_drivers_profile_pic} alt="" />
				<h3>{detailWallet.get_drivers_name}</h3>
			</div>
