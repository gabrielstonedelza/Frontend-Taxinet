<script>
	...
	const handleUpdateRequest = async () => {
		...
		const apiUrl = `https://taxinetghana.xyz/admin_update_passenger_profile/${detailId}/`;
		axios({
			method: 'PUT',
			url: apiUrl,
			data: {
			...
			.then((response) => {
				updateSuccess = 'User was verified successfully.';
				isUpdated = true;
				handleAddToVeried();
			})
			.catch((error) => {
	...
	const handleAddToVeried = async () => {
		isPosting = true;
		setTimeout(function () {
			isPosting = false;
		}, 1200);
		const apiUrl = `https://taxinetghana.xyz/add_to_verified_profile/`;
		axios({
			method: 'POST',
			url: apiUrl,
			data: {
...
<div class="mainformbox">
	...
	>
		<form class="form" on:submit|preventDefault={handleUpdateRequest}>
			<h1 class="form__title">Update {detailId}</h1>

			<div class="form__group">
				<select name="categories" bind:value={verified} class="form__input">
					<option value="False">False</option>
					<option value="True">True</option>
				</select>
				<label for="poster" class="form__label">Verify User</label>
			</div>
			{#if isPosting}
				<Spinner />
			{:else if !isUpdated}
			...
			<br />
			{#if isUpdated}
				<a sveltekit:reload href={`/passenger/${detailId}/`}>Back</a>
			{/if}
		</form>
		{#if !isUpdated}
