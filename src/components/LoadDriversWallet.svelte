<script>
	...
	let isUpdated = false;
	let amount = '';
	let driverHasWalletError = false;
	...
	const handleLoadWallet = async () => {
		...
		const apiUrl = `https://taxinetghana.xyz/admin_load_drivers_wallet/`;
		axios({
			method: 'POST',
			url: apiUrl,
			data: {
...
<div class="mainformbox">
	...
	>
		<form class="form" on:submit|preventDefault={handleLoadWallet}>
			...
			{#if isUpdated}
				<a sveltekit:reload href="/driverswallets">Back</a>
			{/if}
			<br />
			{#if driverHasWalletError}
